...
export default class InfinityMenu extends React.Component {
  ...
  */
  ...
  setDisplayTree(tree, prevs, curr, keyPath) {
    ...
    const nodeLevel = Math.floor(keyPath.split(".").length / 2);
    const isNodeParent = !curr.children;
    const isNodeCurrent = curr.isCurrent === undefined ? false : curr.isCurrent;


    // set node params
    curr.keyPath = keyPath;
    curr.level = nodeLevel;
    curr.isParent = isNodeParent;
    curr.isCurrent = isNodeCurrent;
    /* the leaves */
    if (!curr.children) {
      const keyPathArray = keyPath.split(".");
      const parentPath = Object.assign([], keyPathArray).splice(0, keyPathArray.length - 2);
...
InfinityMenu.propTypes = {
  ...
  customChildrenComponentMappings: PropTypes.object,
